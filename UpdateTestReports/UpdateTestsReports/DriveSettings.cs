...
namespace UpTestDater
{
    public class DriveSettings
    {
        ...
        {
            string sDriveLetter = new string(GetNextAvailableDriveLetter(), 1);
        ...
        private static char GetNextAvailableDriveLetter()
        {
            ...
            for (char i = 'C'; i <= 'Z'; i++) // increment from ASCII values for A-Z
            {
                driveLetters.Add(i); // Add uppercase letters to possible drive letters
            }

            foreach (string drive in Directory.GetLogicalDrives())
            {
                char x = drive.ToCharArray()[0];
                if (driveLetters.Contains(x))
                {
            ...
            return driveLetters[driveLetters.Count - 1];
        }
